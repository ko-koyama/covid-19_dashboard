...
export async function loadInfectionTrendData() {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}data/01_kansensyasuu_suii.csv`);
...
export async function loadAgeGroupData() {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}data/02_kansensyasuu_nendaibetsu.csv`);
    const text = await response.text();
    const data = csvParse(text);
...
export async function loadDeathData() {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}data/07_shibousyasuu.csv`);
    const text = await response.text();
    const data = csvParse(text);
